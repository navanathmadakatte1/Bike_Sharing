...
st.sidebar.markdown('<p class="centered-subheader">User Input Parameters</p>', unsafe_allow_html=True)
...
def predict_bike_counts(model, model_name):
    ...
    st.markdown(
        ...
        <div style="background-color: {twitch_color}; padding: 10px; border-radius: 5px;">
            <h2 style="color: white; text-align: center; font-size: {text_size};"> Predicted Bike Counts (Original Scale): {predicted_counts_original_scale[0]}</h2>
        </div>
        """,
...
# XGBoost
st.markdown('<p class="centered-subheader">The predicted bike demand for the selected parameters is:</p>', unsafe_allow_html=True)
